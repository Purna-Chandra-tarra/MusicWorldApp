...
class TrendAlubmlist extends StatefulWidget {
  final TrendAlbum trendAlbum;
  final Song? song;
  //final void Function(Song song) onToggleFavorite;
  const TrendAlubmlist({super.key, required this.trendAlbum,this.song });

  @override
  State<TrendAlubmlist> createState() => _TrendAlubmlistState();
}

class _TrendAlubmlistState extends State<TrendAlubmlist> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchSongs() async {
    try {
      print(widget.trendAlbum.id);
      final response = await http.get(Uri.parse('http://jiosavvan.vercel.app/albums?id=${widget.trendAlbum.id}'));
          ...
          Scaffold(
            ...
            body: Padding(
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    if (widget.trendAlbum != null &&
                          widget.trendAlbum.image.isNotEmpty)
                        ClipRRect(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              image: DecorationImage(
                                image: NetworkImage(
                                    widget.trendAlbum.image.last['link']),
                        ...
                        SizedBox(height: 10,),
                        Text(widget.trendAlbum.name, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.red),),
                    Expanded(
                      child: songs.isEmpty
                          ? Center(
